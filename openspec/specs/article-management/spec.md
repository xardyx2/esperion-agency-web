...
This specification defines the article management capabilities for the Esperion agency web platform, including multi-language translation support, slug generation, and translation workflows.
...
│     → translation_status recalculated from available content│
│                                                             │
...
│     → Language switcher keeps article context               │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
async function switchLanguage(newLang: 'id' | 'en') {
  ...
  if (targetSlug) {
    ...
    // Keep current page when translation is unavailable
    return
  }
}
```
...
### Scenario: User switches language on article detail page

- **WHEN** a user is on `/id/articles/apa-itu-agency`
- **AND** clicks the language switcher to English
- **THEN** the system resolves translation mapping by slug
- **AND** redirects to `/en/articles/what-is-agency` when translation exists
...
- **AND** current article URL remains unchanged
...
- **AND** translation status is recalculated from available language content
