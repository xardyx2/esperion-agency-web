        ...
        <div class="flex items-start gap-4">
          ...
          <div>
            ...
            <p class="text-es-text-secondary dark:text-es-text-secondary-dark text-sm">
              Digital marketing expert with 10+ years of experience helping businesses grow their online presence.
            </p>
          </div>
        </div>
      </div>
...
// Sample static data for article - in a real app, this would come from an API
const article = ref({
...
const articleKeywords = computed(() => {
  return article.value.title.toLowerCase()
    .replace(/[^\w\s]|_/g, '')
    .replace(/\s+/g, ' ')
    .split(' ')
    .slice(0, 5)
...
// Generate SEO meta dynamically
const pageUrl = `https://esperion.one/id/articles/${article.value.slug_id}`
const imageUrl = `https://esperion.one${article.value.image}`
...
useSeoMeta({
  ...
  link: [
    { rel: 'alternate', hreflang: 'id', href: pageUrl },
    { rel: 'alternate', hreflang: 'en', href: pageUrl.replace('/id/', '/en/') },
    { rel: 'alternate', hreflang: 'x-default', href: pageUrl }
  ]
})

// Define Article Schema with Indonesian context
useSchemaOrg([
  defineArticle({
    ...
    'author': {
      ...
      'url': 'https://esperion.one/id/experts/' + article.value.author.toLowerCase().replace(/\s+/g, '-')
    },
    'publisher': {
      '@type': 'Organization',
  ...
  defineBreadcrumbList({
    itemListElement: [
      ...
      },
      {
        ...
        'position': 2,
        'name': 'Artikel',
        'item': 'https://esperion.one/id/articles'
      },
      {
        '@type': 'ListItem',
        'position': 3,
