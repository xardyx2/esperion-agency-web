import { _ as __nuxt_component_0 } from './nuxt-link-B4_Aso1E.mjs'
import { defineComponent, computed, mergeProps, unref, withCtx, openBlock, createBlock, createVNode, createTextVNode, toDisplayString, useSSRContext } from 'vue'
import { ssrRenderAttrs, ssrRenderClass, ssrInterpolate, ssrRenderList, ssrRenderComponent } from 'vue/server-renderer'
import { _ as _export_sfc, b as useI18n, a as useLocalePath, c as useSeoMeta } from './server.mjs'
import '../_/nitro.mjs'
import 'node:http'
import 'node:https'
import 'node:events'
import 'node:buffer'
import 'consola'
import 'uncsrf'
import 'vue-router'
import 'lru-cache'
import 'node:fs'
import 'node:path'
import 'node:url'
import '@iconify/utils'
import 'node:crypto'
import 'fast-xml-parser'
import 'xss'
import 'zod'
import 'ms'
import 'node:fs/promises'
import '@modelcontextprotocol/sdk/server/mcp.js'
import '@modelcontextprotocol/sdk/server/streamableHttp.js'
import 'satori'
import 'ipx'
import 'pinia'
import '@vue/shared'
import '@unhead/schema-org/vue'
import 'tailwindcss/colors'
import '@iconify/vue'
import '../routes/renderer.mjs'
import 'vue-bundle-renderer/runtime'
import 'unhead/server'
import 'devalue'
import 'unhead/plugins'
import 'unhead/utils'

const _sfc_main = /* @__PURE__ */ defineComponent({
  __name: 'privacy-policy',
  __ssrInlineRender: true,
  setup(__props) {
    const { t, locale } = useI18n()
    const localePath = useLocalePath()
    useSeoMeta({
      title: () => locale.value === 'id' ? 'Kebijakan Privasi - Esperion' : 'Privacy Policy - Esperion',
      description: () => locale.value === 'id' ? 'Kebijakan privasi Esperion Digital Agency' : 'Privacy policy of Esperion Digital Agency'
    })
    const content = computed(() => ({
      id: {
        title: 'Kebijakan Privasi',
        lastUpdated: 'Terakhir diperbarui: 3 Mei 2026',
        sections: [
          {
            title: '1. Informasi yang Kami Kumpulkan',
            content: `
          Kami mengumpulkan berbagai jenis informasi untuk menyediakan dan meningkatkan layanan kami:
          
          **Informasi yang Anda Berikan:**
          - Nama lengkap dan nama perusahaan
          - Alamat email dan nomor telepon
          - Konten formulir kontak yang Anda kirimkan
          
          **Informasi yang Dikumpulkan Secara Otomatis:**
          - Data analitik (menggunakan Google Analytics 4)
          - Perilaku browsing (menggunakan Microsoft Clarity untuk heatmaps)
          - Cookie dan teknologi pelacakan serupa
        `
          },
          {
            title: '2. Bagaimana Kami Menggunakan Informasi Anda',
            content: `
          Kami menggunakan informasi yang dikumpulkan untuk:
          
          - Merespons pertanyaan dan permintaan Anda
          - Mengirimkan informasi yang Anda minta
          - Meningkatkan website dan layanan kami
          - Menganalisis penggunaan website untuk optimasi
          - Mematuhi kewajiban hukum
        `
          },
          {
            title: '3. Berbagi Informasi',
            content: `
          Kami **TIDAK** menjual atau menyewakan informasi pribadi Anda kepada pihak ketiga.
          
          Kami dapat berbagi informasi dengan:
          
          - **Penyedia Layanan**: Google Analytics, Microsoft Clarity untuk analitik
          - **Kewajiban Hukum**: Jika diwajibkan oleh hukum atau peraturan
          - **Persetujuan Anda**: Dengan persetujuan eksplisit Anda
        `
          },
          {
            title: '4. Cookie dan Persetujuan (Consent)',
            content: `
          Website kami menggunakan cookie dan sistem manajemen persetujuan untuk melindungi privasi Anda:
          
          **Sistem Persetujuan (Consent Management)**
          Kami menggunakan sistem consent berbasis tier yang memungkinkan Anda memilih cookie mana yang ingin diaktifkan:
          
          - **Essential (Wajib)**: Cookie yang diperlukan untuk fungsi dasar website. Tidak dapat dinonaktifkan.
          - **Analytics**: Membantu kami memahami cara penggunaan website (Google Analytics 4, GTM)
          - **Functional**: Meningkatkan pengalaman pengguna (Microsoft Clarity untuk heatmaps)
          - **Marketing**: Hanya diaktifkan dengan persetujuan Anda (Meta Pixel, TikTok Pixel, LinkedIn Pixel)
          
          **Mengelola Preferensi**
          Anda dapat mengubah preferensi consent kapan saja dengan mengklik tombol "Cookie Settings" di footer website kami. 
          Persetujuan Anda akan disimpan selama 6 bulan dan dapat diubah kapan saja.
          
          **Browser Controls**
          Anda juga dapat mengontrol pengaturan cookie melalui browser Anda.
        `
          },
          {
            title: '5. Hak Anda (GDPR)',
            content: `
          Jika Anda berada di Wilayah Ekonomi Eropa (EEA), Anda memiliki hak:
          
          - **Akses**: Meminta salinan data pribadi Anda
          - **Koreksi**: Meminta perbaikan data yang tidak akurat
          - **Penghapusan**: Meminta penghapusan data pribadi Anda
          - **Pembatasan**: Meminta pembatasan pemrosesan data
          - **Portabilitas**: Menerima data dalam format terstruktur
          - **Keberatan**: Menolak pemrosesan data tertentu
          
           Untuk menggunakan hak ini, hubungi: privacy@esperion.one
        `
          },
          {
            title: '6. Keamanan Data',
            content: `
          Kami mengambil langkah-langkah keamanan untuk melindungi informasi Anda:
          
          - Enkripsi data dalam transmisi (SSL/TLS)
          - Akses terbatas ke informasi pribadi
          - Review berkala praktik pengumpulan data
          - Penyimpanan data di server yang aman
          
          Namun, tidak ada metode transmisi internet yang 100% aman.
        `
          },
          {
            title: '7. Retensi Data',
            content: `
          Kami menyimpan informasi pribadi Anda selama:
          
          - **Data Kontak**: 3 tahun setelah interaksi terakhir
          - **Data Analitik**: 14 bulan (Google Analytics default)
          - **Session Recordings**: 30 hari (Microsoft Clarity)
          
          Setelah periode ini, data akan dihapus atau dianonimkan.
        `
          },
          {
            title: '8. Perubahan Kebijakan Privasi',
            content: `
          Kami dapat memperbarui kebijakan privasi ini dari waktu ke waktu.
          Perubahan akan diposting di halaman ini dengan tanggal pembaruan.
          
          Kami mendorong Anda untuk meninjau kebijakan ini secara berkala.
        `
          },
          {
            title: '9. Hubungi Kami',
            content: `
          Jika Anda memiliki pertanyaan tentang kebijakan privasi ini:
          
           **Email**: privacy@esperion.one
           **Alamat**: Detail alamat publik menunggu konfirmasi final
           **Telepon**: Detail telepon legal menunggu konfirmasi final
          
          Untuk keluhan, Anda dapat menghubungi otoritas perlindungan data setempat.
        `
          }
        ]
      },
      en: {
        title: 'Privacy Policy',
        lastUpdated: 'Last updated: May 3, 2026',
        sections: [
          {
            title: '1. Information We Collect',
            content: `
          We collect various types of information to provide and improve our services:
          
          **Information You Provide:**
          - Full name and company name
          - Email address and phone number
          - Contact form content you submit
          
          **Information Collected Automatically:**
          - Analytics data (using Google Analytics 4)
          - Browsing behavior (using Microsoft Clarity for heatmaps)
          - Cookies and similar tracking technologies
        `
          },
          {
            title: '2. How We Use Your Information',
            content: `
          We use the collected information to:
          
          - Respond to your inquiries and requests
          - Send information you requested
          - Improve our website and services
          - Analyze website usage for optimization
          - Comply with legal obligations
        `
          },
          {
            title: '3. Information Sharing',
            content: `
          We do **NOT** sell or rent your personal information to third parties.
          
          We may share information with:
          
          - **Service Providers**: Google Analytics, Microsoft Clarity for analytics
          - **Legal Requirements**: If required by law or regulation
          - **Your Consent**: With your explicit consent
        `
          },
          {
            title: '4. Cookies and Consent',
            content: `
          Our website uses cookies and a consent management system to protect your privacy:
          
          **Consent Management System**
          We use a tiered consent system that allows you to choose which cookies to enable:
          
          - **Essential (Required)**: Cookies required for basic website functionality. Cannot be disabled.
          - **Analytics**: Help us understand website usage (Google Analytics 4, GTM)
          - **Functional**: Enhance user experience (Microsoft Clarity for heatmaps)
          - **Marketing**: Only activated with your consent (Meta Pixel, TikTok Pixel, LinkedIn Pixel)
          
          **Managing Preferences**
          You can change your consent preferences at any time by clicking the "Cookie Settings" button in our website footer. 
          Your consent will be stored for 6 months and can be changed at any time.
          
          **Browser Controls**
          You can also control cookie settings through your browser.
        `
          },
          {
            title: '5. Your Rights (GDPR)',
            content: `
          If you are in the European Economic Area (EEA), you have the right to:
          
          - **Access**: Request a copy of your personal data
          - **Correction**: Request correction of inaccurate data
          - **Deletion**: Request deletion of your personal data
          - **Restriction**: Request restriction of data processing
          - **Portability**: Receive data in a structured format
          - **Objection**: Object to certain data processing
          
           To exercise these rights, contact: privacy@esperion.one
        `
          },
          {
            title: '6. Data Security',
            content: `
          We take security measures to protect your information:
          
          - Data encryption in transit (SSL/TLS)
          - Limited access to personal information
          - Regular review of data collection practices
          - Storage on secure servers
          
          However, no internet transmission method is 100% secure.
        `
          },
          {
            title: '7. Data Retention',
            content: `
          We retain personal information for:
          
          - **Contact Data**: 3 years after last interaction
          - **Analytics Data**: 14 months (Google Analytics default)
          - **Session Recordings**: 30 days (Microsoft Clarity)
          
          After these periods, data will be deleted or anonymized.
        `
          },
          {
            title: '8. Changes to This Policy',
            content: `
          We may update this privacy policy from time to time.
          Changes will be posted on this page with an updated date.
          
          We encourage you to review this policy periodically.
        `
          },
          {
            title: '9. Contact Us',
            content: `
          If you have questions about this privacy policy:
          
           **Email**: privacy@esperion.one
           **Address**: Public address details are pending final confirmation
           **Phone**: Legal contact number is pending final confirmation
          
          For complaints, you may contact your local data protection authority.
        `
          }
        ]
      }
    }))
    const currentContent = computed(() => content.value[locale.value])
    return (_ctx, _push, _parent, _attrs) => {
      const _component_NuxtLink = __nuxt_component_0
      _push(`<div${ssrRenderAttrs(mergeProps({ class: 'min-h-screen py-12' }, _attrs))} data-v-9a4a568b><div class="container mx-auto px-4 max-w-4xl" data-v-9a4a568b><header class="mb-8" data-v-9a4a568b><h1 class="${ssrRenderClass([[
        'text-es-text-primary dark:text-es-text-primary-dark'
      ], 'text-4xl font-bold mb-4'])}" data-v-9a4a568b>${ssrInterpolate(unref(currentContent).title)}</h1><p class="${ssrRenderClass([[
        'text-es-text-secondary dark:text-es-text-secondary-dark'
      ], 'text-sm'])}" data-v-9a4a568b>${ssrInterpolate(unref(currentContent).lastUpdated)}</p></header><article class="prose prose-lg dark:prose-invert max-w-none" data-v-9a4a568b><!--[-->`)
      ssrRenderList(unref(currentContent).sections, (section) => {
        _push(`<section class="mb-8" data-v-9a4a568b><h2 class="${ssrRenderClass([[
          'text-es-text-primary dark:text-es-text-primary-dark'
        ], 'text-2xl font-semibold mb-4'])}" data-v-9a4a568b>${ssrInterpolate(section.title)}</h2><div class="${ssrRenderClass([[
          'text-es-text-secondary dark:text-es-text-secondary-dark'
        ], 'whitespace-pre-line'])}" data-v-9a4a568b>${section.content ?? ''}</div></section>`)
      })
      _push(`<!--]--></article><div class="mt-12" data-v-9a4a568b>`)
      _push(ssrRenderComponent(_component_NuxtLink, {
        to: unref(localePath)('/'),
        class: 'inline-flex items-center gap-2 text-es-accent-primary dark:text-es-accent-primary-dark hover:underline'
      }, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" data-v-9a4a568b${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" data-v-9a4a568b${_scopeId}></path></svg> ${ssrInterpolate(unref(locale) === 'id' ? 'Kembali ke Beranda' : 'Back to Home')}`)
          } else {
            return [
              (openBlock(), createBlock('svg', {
                class: 'w-5 h-5',
                fill: 'none',
                stroke: 'currentColor',
                viewBox: '0 0 24 24'
              }, [
                createVNode('path', {
                  'stroke-linecap': 'round',
                  'stroke-linejoin': 'round',
                  'stroke-width': '2',
                  'd': 'M10 19l-7-7m0 0l7-7m-7 7h18'
                })
              ])),
              createTextVNode(' ' + toDisplayString(unref(locale) === 'id' ? 'Kembali ke Beranda' : 'Back to Home'), 1)
            ]
          }
        }),
        _: 1
      }, _parent))
      _push(`</div></div></div>`)
    }
  }
})
const _sfc_setup = _sfc_main.setup
_sfc_main.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add('app/pages/privacy-policy.vue')
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0
}
const privacyPolicy = /* @__PURE__ */ _export_sfc(_sfc_main, [['__scopeId', 'data-v-9a4a568b']])

export { privacyPolicy as default }
// # sourceMappingURL=privacy-policy-CyD_ywAE.mjs.map
