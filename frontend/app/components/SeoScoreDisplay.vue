<template>
  <div class="bg-es-bg-secondary dark:bg-es-bg-secondary-dark rounded-xl p-6">
    ...
    <!-- Suggestions -->
    <div
      v-if="seoScore && seoScore.suggestions.length > 0"
      class="mt-6 pt-6 border-t border-es-border dark:border-es-border-dark"
    >
      <h4 class="text-sm font-semibold text-es-text-primary dark:text-es-text-primary-dark mb-3">
        Suggestions for Improvement
      </h4>
      <ul class="space-y-2">
        <li
          v-for="(suggestion, index) in seoScore.suggestions"
          :key="index"
    ...
    >
      ...
      <p class="text-es-text-secondary dark:text-es-text-secondary-dark">
        No SEO score available yet
      </p>
      <button
        class="mt-4 px-4 py-2 bg-es-accent-primary dark:bg-es-accent-primary-dark text-es-text-inverse dark:text-es-text-inverse-dark rounded-lg text-sm font-medium hover:bg-es-accent-primary-hover dark:hover:bg-es-accent-primary-hover-dark transition-colors"
