<template>
  <div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
      <div>
        <h1 class="text-2xl md:text-3xl font-bold text-es-text-primary dark:text-es-text-primary-dark mb-2">{{ t('dashboard.services.title') }}</h1>
        <p class="text-es-text-secondary dark:text-es-text-secondary-dark">{{ t('dashboard.services.description') }}</p>
      </div>
    ...
    <section v-if="showForm" class="rounded-xl border border-es-border bg-es-bg-secondary p-5 shadow-sm dark:border-es-border-dark dark:bg-es-bg-secondary-dark">
      <div class="mb-4 flex items-center justify-between gap-4">
        <div>
          <h2 class="text-lg font-semibold text-es-text-primary dark:text-es-text-primary-dark">{{ editingServiceId ? t('dashboard.services.create.update') : t('dashboard.services.create.title') }}</h2>
          <p class="text-sm text-es-text-secondary dark:text-es-text-secondary-dark">{{ editingServiceId ? 'Update retained service catalog fields.' : t('dashboard.services.create.title') }}</p>
        </div>
      ...
      <form class="grid gap-4 md:grid-cols-2" @submit.prevent="submitForm">
        <label class="space-y-2 text-sm md:col-span-2">
          ...
          <input v-model="form.title" type="text" required class="w-full rounded-lg border border-es-border bg-es-bg-primary px-4 py-3 text-es-text-primary focus:outline-none focus:ring-2 focus:ring-es-accent-primary dark:border-es-border-dark dark:bg-es-bg-primary-dark dark:text-es-text-primary-dark" />
        </label>
        ...
        <label class="space-y-2 text-sm">
          ...
          <input v-model.number="form.display_order" type="number" min="0" class="w-full rounded-lg border border-es-border bg-es-bg-primary px-4 py-3 text-es-text-primary focus:outline-none focus:ring-2 focus:ring-es-accent-primary dark:border-es-border-dark dark:bg-es-bg-primary-dark dark:text-es-text-primary-dark" />
        </label>
        ...
        <label class="space-y-2 text-sm md:col-span-2">
          ...
          <textarea v-model="form.description" rows="4" required class="w-full rounded-lg border border-es-border bg-es-bg-primary px-4 py-3 text-es-text-primary focus:outline-none focus:ring-2 focus:ring-es-accent-primary dark:border-es-border-dark dark:bg-es-bg-primary-dark dark:text-es-text-primary-dark" />
        </label>
        ...
        <label class="space-y-2 text-sm">
          ...
          <input v-model="form.icon" type="text" placeholder="Brief icon label" class="w-full rounded-lg border border-es-border bg-es-bg-primary px-4 py-3 text-es-text-primary focus:outline-none focus:ring-2 focus:ring-es-accent-primary dark:border-es-border-dark dark:bg-es-bg-primary-dark dark:text-es-text-primary-dark" />
        </label>
    ...
    <div v-else class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div v-for="service in services" :key="service.id" class="bg-es-bg-secondary dark:bg-es-bg-secondary-dark rounded-xl p-6">
        <div class="text-sm mb-4 text-es-text-secondary dark:text-es-text-secondary-dark">{{ service.icon || t('dashboard.services.buttons.noIcon') }}</div>
        <h3 class="font-semibold text-es-text-primary dark:text-es-text-primary-dark mb-2">{{ service.title }}</h3>
        <p class="text-sm text-es-text-secondary dark:text-es-text-secondary-dark mb-4 line-clamp-2">{{ service.description }}</p>
        <div class="flex items-center justify-between">
          <span v-if="service.featured" class="px-2 py-1 bg-yellow-500/10 text-yellow-500 text-xs rounded-full">{{ t('dashboard.services.buttons.featured') }}</span>
          <div class="flex items-center gap-2 ml-auto">
            <button type="button" class="p-2 hover:bg-es-bg-tertiary dark:hover:bg-es-bg-tertiary-dark rounded-lg" @click="openEdit(service)">{{ t('dashboard.services.buttons.edit') }}</button>
            <button type="button" class="p-2 hover:bg-es-bg-tertiary dark:hover:bg-es-bg-tertiary-dark rounded-lg" @click="removeService(service)">{{ t('dashboard.services.buttons.delete') }}</button>
          </div>
        </div>
      </div>

      <div v-if="!services.length" class="col-span-full rounded-xl border border-es-border bg-es-bg-secondary px-4 py-8 text-center text-sm text-es-text-secondary dark:border-es-border-dark dark:bg-es-bg-secondary-dark dark:text-es-text-secondary-dark">
