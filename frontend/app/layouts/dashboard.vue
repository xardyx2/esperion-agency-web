  ...
  <div class="min-h-screen bg-es-bg-primary dark:bg-es-bg-primary-dark">
    ...
    >
      ...
      <div class="flex items-center justify-between h-16 px-4 border-b border-es-border dark:border-es-border-dark">
        <NuxtLink to="/dashboard" class="text-xl font-bold text-es-text-primary dark:text-es-text-primary-dark">
          Esperion
        </NuxtLink>
        <button @click="sidebarOpen = false" class="lg:hidden p-2 rounded-lg hover:bg-es-bg-tertiary dark:hover:bg-es-bg-tertiary-dark">
      ...
      <div class="absolute bottom-0 left-0 right-0 p-4 border-t border-es-border dark:border-es-border-dark">
        ...
        >
          ...
          <option value="capital">Capital Dashboard</option>
        </select>
      </div>
    </aside>
    ...
    <div class="lg:pl-64">
      ...
      <header class="sticky top-0 z-40 bg-es-bg-primary dark:bg-es-bg-primary-dark border-b border-es-border dark:border-es-border-dark">
        <div class="flex items-center justify-between h-16 px-4">
          <!-- Mobile Menu Button -->
          <button @click="sidebarOpen = true" class="lg:hidden p-2 rounded-lg hover:bg-es-bg-tertiary dark:hover:bg-es-bg-tertiary-dark">
            <span class="text-2xl">☰</span>
          </button>

          <!-- Breadcrumb -->
          <div class="hidden sm:flex items-center gap-2 text-sm text-es-text-secondary dark:text-es-text-secondary-dark">
            <NuxtLink to="/dashboard" class="hover:text-es-accent-primary dark:hover:text-es-accent-primary-dark">Dashboard</NuxtLink>
            <span>/</span>
          ...
          <div class="flex items-center gap-4">
            ...
            <button class="relative p-2 rounded-lg hover:bg-es-bg-tertiary dark:hover:bg-es-bg-tertiary-dark transition-colors">
              <span class="text-xl">🔔</span>
              <span class="absolute top-1 right-1 w-2 h-2 bg-es-accent-primary rounded-full"></span>
            </button>

            <!-- User Menu -->
            <div class="flex items-center gap-3">
              <div class="hidden sm:block text-right">
                <div class="text-sm font-medium text-es-text-primary dark:text-es-text-primary-dark">{{ user.name }}</div>
                <div class="text-xs text-es-text-secondary dark:text-es-text-secondary-dark">{{ user.role }}</div>
              </div>
